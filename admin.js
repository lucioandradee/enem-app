...
// ── Config ──────────────────────────────────────────────────────
const ADMIN_EMAILS      = ['levyd604@gmail.com'];   // único admin autorizado
...
async function bootApp() {
    ...
    if (verifyErr || !verifiedUser || !ADMIN_EMAILS.includes(verifiedUser.email)) {
        await supabase.auth.signOut();
        adminUser = null;
        const errEl = document.getElementById('gate-error');
        if (errEl) {
            errEl.textContent   = 'Acesso negado: conta não autorizada.';
            errEl.style.display = 'block';
        }
        const btn    = document.getElementById('gate-btn');
        const btnTxt = document.getElementById('gate-btn-text');
